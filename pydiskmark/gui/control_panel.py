...
class ControlPanel(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        # Use grid layout — 2 columns: label | combo
        row = 0

        # --- Profile ---
        ttk.Label(self, text="Profile").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        self._profile_var = tk.StringVar()
        profiles = [p.symbol for p in BenchmarkProfile.get_defaults()]
        self._profile_combo = ttk.Combobox(
            self, textvariable=self._profile_var,
            values=profiles, state="readonly", width=18,
        )
        self._profile_combo.grid(row=row, column=1, sticky="ew", padx=5, pady=2)
        self._profile_combo.bind("<<ComboboxSelected>>", self._on_profile_change)
        row += 1

        # --- Type ---
        ttk.Label(self, text="Type").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        self._type_var = tk.StringVar()
        self._type_combo = ttk.Combobox(
            self, textvariable=self._type_var,
            values=[t.value for t in BenchmarkType], state="readonly", width=18,
        )
        self._type_combo.grid(row=row, column=1, sticky="ew", padx=5, pady=2)
        row += 1

        # --- Threads ---
        ttk.Label(self, text="Threads").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        self._threads_var = tk.StringVar()
        self._threads_combo = ttk.Combobox(
            self, textvariable=self._threads_var,
            values=[str(t) for t in THREAD_OPTIONS], state="readonly", width=18,
        )
        self._threads_combo.grid(row=row, column=1, sticky="ew", padx=5, pady=2)
        row += 1

        # --- Order ---
        ttk.Label(self, text="Block Order").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        self._order_var = tk.StringVar()
        self._order_combo = ttk.Combobox(
            self, textvariable=self._order_var,
            values=[s.name for s in BlockSequence], state="readonly", width=18,
        )
        self._order_combo.grid(row=row, column=1, sticky="ew", padx=5, pady=2)
        row += 1

        # --- Blocks ---
        ttk.Label(self, text="Blocks / Sample").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        self._blocks_var = tk.StringVar()
        self._blocks_combo = ttk.Combobox(
            self, textvariable=self._blocks_var,
            values=[str(b) for b in BLOCK_COUNT_OPTIONS], state="readonly", width=18,
        )
        self._blocks_combo.grid(row=row, column=1, sticky="ew", padx=5, pady=2)
        row += 1

        # --- Block Size ---
        ttk.Label(self, text="Block Size (KB)").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        self._block_size_var = tk.StringVar()
        self._block_size_combo = ttk.Combobox(
            self, textvariable=self._block_size_var,
            values=[str(s) for s in BLOCK_SIZE_OPTIONS], state="readonly", width=18,
        )
        self._block_size_combo.grid(row=row, column=1, sticky="ew", padx=5, pady=2)
        row += 1

        # --- Samples ---
        ttk.Label(self, text="Samples").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        self._samples_var = tk.StringVar()
        self._samples_combo = ttk.Combobox(
            self, textvariable=self._samples_var,
            values=[str(s) for s in SAMPLE_OPTIONS], state="readonly", width=18,
        )
        self._samples_combo.grid(row=row, column=1, sticky="ew", padx=5, pady=2)
        row += 1

        # --- Start / Stop Button ---
        self._start_btn = ttk.Button(self, text="Start", command=self._on_start_stop)
        self._start_btn.grid(row=row, column=0, columnspan=2, sticky="ew", padx=5, pady=(10, 5))
        row += 1
        ...
        # --- Results grid: 3 columns (Metric | Write | Read) ---
        results_frame = ttk.Frame(self)
        results_frame.grid(row=row, column=0, columnspan=2, sticky="ew", padx=5)
        ...
        # Bandwidth row
        ttk.Label(results_frame, text="MB/s").grid(row=1, column=0, sticky="w", padx=3, pady=1)
        ...
        # Latency row
        ttk.Label(results_frame, text="Lat (ms)").grid(row=2, column=0, sticky="w", padx=3, pady=1)
        ...
        # Make columns expand evenly in results
        results_frame.columnconfigure(1, weight=1)
        results_frame.columnconfigure(2, weight=1)

        # Make the combo column expand
    ...
    def _on_profile_change(self, _event=None) -> None:
        ...
        symbol = self._profile_var.get()
        try:
            profile = BenchmarkProfile.from_symbol(symbol)
        except ValueError:
            return
    ...
    def refresh_from_app(self) -> None:
        ...
        self._profile_var.set(app.active_profile.symbol)
        self._type_var.set(app.benchmark_type.value)
        self._threads_var.set(str(app.num_of_threads))
        self._order_var.set(app.block_sequence.name)
